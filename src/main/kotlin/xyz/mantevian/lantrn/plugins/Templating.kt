...
fun Application.configureTemplating() {
	...
	for (i in 1..5) {
		articles.add(
			Article(
				i, "hello world", Date(), "this is a description",
	...
	routing {
		...
		get("/generate") {
			...
			val seed = call.parameters["seed"]?.toIntOrNull() ?: Random.nextInt()

			when (type) {
				"simple_grid" -> call.respond(Encoder.encode(SimpleGridGenerator(seed).generate(n)))
				"fireflies" -> call.respond(Encoder.encode(FirefliesGenerator(seed).generate(n)))
				"waves" -> call.respond(Encoder.encode(WavesGenerator(seed).generate(n)))
			}
		}
	}
}
